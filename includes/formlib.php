 ...
 */
function formulario($formId, $generaCamposFormulario, $procesaFormulario, $action=null, $class=null) {
  ...
  if ( ! formularioEnviado($_POST, $formId) ) {
    echo generaFormulario($formId, $generaCamposFormulario, $action, $class);
  } else {
    ...
    if ( ($errores = csrfguard_ValidateToken($formId, $tokenRecibido)) !== TRUE ) { 
      echo generaFormulario($formId, $generaCamposFormulario, $action, $class, $errores, $_POST);
    } else  {
      $result = $procesaFormulario($_POST);
      if ( is_array($result) ) {
        // Error al procesar el formulario, volvemos a mostrarlo
        echo generaFormulario($formId, $generaCamposFormulario, $action, $class, $result, $_POST);
      } else {
 ...
 */
function generaFormulario($formId, $generaCamposFormulario, $action, $class, $errores = array(), &$datos = array()) {
